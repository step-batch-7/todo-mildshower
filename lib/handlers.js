...
const TODO_LISTS = TodoListCollection.load(require(TODO_DATA_STORE_PATH));
...
const addTodoList = function(req, res) {
  ...
  const newTodoListJson = TODO_LISTS.addTodoList(title, taskNames);
...
const deleteTodoList = function(req, res){
  const {todoListId} = JSON.parse(req.body);
...
const addTaskToTodo = function(req, res) {
  const {todoListId, taskName} = JSON.parse(req.body);
