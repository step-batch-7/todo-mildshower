...
const acceptLoginCredentials = function(res, userName){
...
const login = function(req, res) {
  ...
  if(userDetails[userName] && userDetails[userName].password === password){
    acceptLoginCredentials(res, userName);
    return;
  }
...
const verifyRequestAuthorization = function(req, res, next){
  if(sessions.isValidSessionId(req.cookies._SID)) {
    return next();
  }
  res.statusCode = 401;
...
app.post(verifyRequestAuthorization);
